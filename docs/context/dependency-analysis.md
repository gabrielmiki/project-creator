...
| `tests/integration/conftest.py` | ✅ **Created (T-016)** → **MODIFY (T-017)** → **MODIFY (T-018)** | Shared fixtures: `AllMixinsPlugin`, `temp_dir`, `minimal_plugin`, `spec_factory` (reuses `tests/unit/_shared.make_spec`), `user_plugin_dir` (flat .py + subdirectory formats), `registry_with_discovery`, `txn`; T-017 adds 7 fixtures: `pipeline_registry`, `validation`, `mock_executor`, `progress`, `orchestrator`, `fastapi_spec`, `cli_spec_json`; T-018 adds 4 fixtures: `qapp` (session-scoped), `full_spec` (fastapi+react with config), `django_htmx_spec` (django+htmx with config), `worker` (qapp-depending GenerationWorker) | `forge.domain`, `forge.infrastructure`, `forge.plugins.base`, `forge.generation.registry`, `forge.infrastructure.transaction`, `tests.unit._shared`, `PySide6.QtWidgets` (qapp) |
...
| `tests/integration/test_cli_headless.py` | ✅ **Created (T-017)** | Exercises `app.main()`, `_run_headless()`, `detect_display()`, `Orchestrator.generate()`, `GenerationTransaction.commit()` | `forge.app`, `forge.generation.orchestrator`, `forge.infrastructure.transaction` |
| `tests/integration/test_orchestrator_pipeline.py` | ✅ **Created (T-017)** | Exercises `Orchestrator.generate()` with real 6 stages, `GenerationTransaction.commit()`/`rollback()`, `MockProgressReporter` | `forge.generation.orchestrator`, `forge.generation.stages.*`, `forge.generation.progress`, `forge.infrastructure.transaction` |
| `tests/integration/test_fastapi_plugin.py` | ✅ **Created (T-017)** | Exercises `PluginExecutionEngine.run()` with real `FastapiPlugin`, real `GenerationTransaction` on real `temp_dir` | `forge.generation.stages.plugin_execution_engine`, `forge.plugins.fastapi.plugin`, `forge.infrastructure.transaction` |
| `tests/integration/test_all_plugins.py` | **CREATE (T-018)** | Multi-plugin pipeline: fastapi+react generation, django+htmx directory coexistence, plugin dependency ordering with custom test-doubles | `forge.generation.orchestrator`, `forge.generation.stages.plugin_execution_engine`, `forge.plugins.fastapi.plugin`, `forge.plugins.react.plugin`, `forge.plugins.django.plugin`, `forge.plugins.htmx.plugin`, `forge.infrastructure.transaction` |
| `tests/integration/test_gui_worker.py` | **CREATE (T-018)** | GUI worker thread: generation on QThread, cancel→rollback, signal routing through MainWindow, finished(success=False) on failure | `forge.ui.workers`, `forge.ui.main_window`, `forge.generation.orchestrator`, `forge.infrastructure.transaction`, `PySide6.QtCore`, `PySide6.QtTest` |
| `tests/integration/test_overwrite_flow.py` | **CREATE (T-018)** | Overwrite confirm dialog: `show_confirm()` at index 3, Yes→proceed, No→stay at review screen | `forge.ui.main_window`, `forge.ui.screens.review_screen`, `forge.ui.screens.generation_screen`, `PySide6.QtWidgets` |
| `tests/integration/test_error_scenarios.py` | **CREATE (T-018)** | Error/rollback: mid-stage exception, scaffold failure/timeout, missing plugin ID (CLI + API), empty project name, special chars deferred (skipped), scaffold+files overlap | `forge.generation.orchestrator`, `forge.generation.validation`, `forge.generation.stages.*`, `forge.infrastructure.transaction`, `forge.app` |
...
| **`FileExistsError` on staged files after pre-population** — `test_orchestrator_overwrite_confirmed_real_stages` pre-populates output dir. `SharedStructureScaffolder` staging `README.md` into an already-populated dir triggers error. Handled via `overwrite_confirmed` skip. | T-017 | **Low** — ticket accounts for it; carefully controlled pre-population |
| **AC-3 `orm="none"` vs actual defaults — `files()` vs `dependencies()` drift risk** — `requirements.txt` is built from inline f-string in `files()`, not from `dependencies()`. If the two methods drift, test only catches the `files()` side. | T-017 | **Low** — well-scoped assertion on `requirements.txt` content |
...
### Detailed Chain: T-017 Integration Tests — CLI + Pipeline

T-017 is the **pipeline integration test ticket** — 3 new test files exercising the full generation pipeline (CLI headless, orchestrator with all 6 real stages, FastAPI plugin through `PluginExecutionEngine`). It is a **test-only ticket**: it modifies zero production files but adds 7 new fixtures to `conftest.py`.

```
T-005 (registry + validation) ───────────────────────────────┐
  PluginRegistry, ValidationEngine                              │
  DiscoveryError, ValidationError                               │
                                                               │
T-006 (all 6 generation stages) ──────────────────────────────┤
  DirectoryInitializer, SharedStructureScaffolder,              ├──► T-017 Integration — CLI + Pipeline
  PluginExecutionEngine, JustfileGenerator,                     │      │
  ProjectDocumentationWriter, AgentSkillScaffolder              │      ├── test_cli_headless.py (15 ACs)
                                                               │      ├── test_orchestrator_pipeline.py (14 ACs)
T-007 (orchestrator + CLI) ───────────────────────────────────┤      └── test_fastapi_plugin.py (13 ACs)
  Orchestrator, GenerationResult, app.main()
  _run_headless, detect_display                                 │
                                                               │
T-008 (FastAPI plugin) ───────────────────────────────────────┤      conftest.py (+7 fixtures):
  FastapiPlugin (4 mixins, 5 methods)                           │      pipeline_registry, validation,
                                                               │      mock_executor, progress,
T-016 (integration foundation) ───────────────────────────────┤      orchestrator, fastapi_spec, cli_spec_json
  temp_dir, txn, spec_factory, minimal_plugin                   │
  user_plugin_dir, registry_with_discovery                      │
```

**Key chain insight:** T-017 is the **first end-to-end pipeline validation** — the first tests that call `app.main()` with a real JSON spec, run all 6 stages in sequence, and verify committed output on the real filesystem. Every upstream ticket (T-005 → T-008) must be implemented and passing for T-017 to work. The `mock_executor` is the **only** mocked component: all registries, stages, transactions, and plugins are real.

**Architecture pattern — canary + pipeline fixtures:**

1. **`pipeline_registry` fixture** (module-scoped) — creates a real `PluginRegistry`, calls `discover()` loading all 4 production plugins (fastapi, django, react, htmx). Any import error in any plugin cascades to fail all T-017 tests. This is an **intentional canary** — same pattern as T-016's `test_entry_point_discovery_loads_production_plugins`.

2. **`mock_executor` fixture** — `unittest.mock.MagicMock(spec=ProcessExecutor)` injected into `Orchestrator.__init__(executor=mock_executor)`. This prevents subprocess calls (`uv add`, `npm create`) during integration tests. `create-fastapi-app` scaffold is similarly stubbed. FastapiPlugin content is verified via `files()` output on disk, not via scaffold commands.

3. **7 new fixtures in conftest.py** — all follow the same lazy-import pattern as T-016's fixtures:
   - `pipeline_registry` — module-scoped `PluginRegistry` + `discover()`
   - `validation` — `ValidationEngine(pipeline_registry)`
   - `mock_executor` — `MagicMock(spec=ProcessExecutor)`
   - `progress` — `MockProgressReporter()`
   - `orchestrator` — `Orchestrator(pipeline_registry, validation, executor=mock_executor)`
   - `fastapi_spec` — `ProjectSpec` with `backend_id="fastapi"` + config variants
   - `cli_spec_json` — JSON string matching `_run_headless()` parser schema

**New test files created:**

| File | Tests | ACs | Exercises |
|------|-------|-----|-----------|
| `tests/integration/test_cli_headless.py` | 5 tests | 15 | `app.main()`, `_run_headless()`, `detect_display()`, JSON parse → validate → generate → commit, error→exit 1 |
| `tests/integration/test_orchestrator_pipeline.py` | 4 tests | 14 | `Orchestrator.generate()` with real 6 stages, commit/rollback, overwrite confirm, MockProgressReporter call records |
| `tests/integration/test_fastapi_plugin.py` | 3 tests | 13 | `PluginExecutionEngine.run()` with real `FastapiPlugin`, config variations (auth, orm, alembic), real txn on real tmp_path |

**Delicate points specific to T-017:**

| # | Point | Risk |
|---|-------|------|
| 1 | **Rollback + staging interaction** — `test_orchestrator_rollback_on_failure` is the first real filesystem rollback test. `txn.rollback()` calls `shutil.rmtree(self.staging)`. Any stage writing files outside `txn.stage_file()` won't be cleaned up. The test correctly asserts `not (output_dir / ".forge-staging").exists()`. | **High** |
| 2 | **Mock executor unused by orchestrator fixture** — the `orchestrator` fixture passes `executor=mock_executor` to `Orchestrator.__init__()`. Looking at the orchestrator code: `self._executor = executor or ProcessExecutor()` — this only sets `self._executor` on the orchestrator. Default stages are built in the same block: `PluginExecutionEngine(registry, self._executor)`. The engine receives the executor. ✅ Correct — but if the `orchestrator` fixture ever passes `stages=...`, the executor is bypassed. | **Medium** |
| 3 | **Mock executor silently swallows `generate()` calls** — `FastapiPlugin.generate()` (which would run `uv add` in production) becomes a no-op. Generated `requirements.txt` comes from `FastapiPlugin.files()`, not from `uv add`. If a future change moves dependency installation from `files()` to `generate()`, integration tests won't detect it. | **Medium** |
| 4 | **Real registry loads all 4 production plugins** — `pipeline_registry` calls `reg.discover()` loading fastapi, django, react, htmx via `importlib.metadata.entry_points()`. Any broken plugin import cascades to fail all T-017 tests. Intentional canary pattern. | **Medium** |
| 5 | **`FileExistsError` on staged files after pre-population** — `test_orchestrator_overwrite_confirmed_real_stages` pre-populates output dir. `SharedStructureScaffolder` staging `README.md` into already-populated dir triggers error. The ticket accounts for this via `overwrite_confirmed` skip, but pre-populated files must be carefully chosen to avoid collision with Stage 4-5 files (justfile, AGENTS.md). | **Low** |
| 6 | **AC-3 `orm="none"` vs `dependencies()` drift** — `config={"fastapi": {"orm": "none"}}` → `FastapiPlugin.files()` skips ORM files. `requirements.txt` is inline in `files()`, not from `dependencies()`. If the two drift, test only catches `files()` side. | **Low** |

**Risks table:**

| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| Registry discovery loads production plugins; any broken plugin fails entire T-017 suite | Medium | High | Document canary; `pytest.skip` guard for individual plugin import failures |
| `mock_executor` silently swallows `generate()` calls | Low | Medium | Tests verify `files()` output on disk, not `generate()` side effects |
| Stage content overlap (SharedStructureScaffolder vs JustfileGenerator) | Low | Low | Strict stage delimitation from T-006; test verifies only expected files exist |
| `cli_spec_json` fixture JSON schema diverges from `_run_headless()` parser | Low | Medium | Fixture is self-contained in conftest; schema matches current parser |
| Module-scoped `pipeline_registry` causes test pollution | Low | Low | Registry is read-only after discovery; no tests modify it |

**Test verification:**
- 42 ACs across 3 new test files — all must PASS
- 7 new fixtures + 6 existing fixtures work together in conftest.py
- `ruff check tests/integration/` — no new lint errors
- 0 regressions in 26 existing integration tests (T-016) and 436+ unit tests

**Implementation status:** ✅ **Complete** — 3 test files created, 1 existing file modified (conftest.py), 0 production code changes. All 42 ACs passing, downstream T-018 depends on this.

---

### Detailed Chain: T-018 Integration Tests — Full Pipeline

T-018 is the **most comprehensive integration test ticket** — it exercises the entire generation pipeline with all 4 production plugins, the GUI worker thread lifecycle, the overwrite confirmation flow, and all error/rollback paths. It is a **leaf test ticket**: it depends on ALL upstream tickets (T-009 through T-017 being implemented and passing) but no downstream ticket depends on it.
...
T-013 (GenerationWorker) ───┤
T-014 (Wizard screens 1-3) ─┤
T-015 (Wizard screens 4-5) ─┴──► T-018 Integration Tests — Full Pipeline
                                      │
T-016 (integration foundation) ───────┤
T-017 (CLI + pipeline) ───────────────┤
                                      │
                                      ├── test_all_plugins.py (4 tests)
                                      ├── test_gui_worker.py (4 tests)
                                      ├── test_overwrite_flow.py (3 tests)
                                      ├── test_error_scenarios.py (7 tests)
                                      └── conftest.py (+4 fixtures: qapp, full_spec,
                                                          django_htmx_spec, worker)
```

**Key chain insight:** T-018 is the **most comprehensive leaf test** — it verifies that every component works together: 4 plugins through `PluginExecutionEngine`, `GenerationWorker` on `QThread` through `MainWindow` signal routing, `GenerationTransaction` commit/rollback on real filesystem I/O, and `ValidationEngine` error paths. Any upstream regression in plugins (T-009–T-011), UI (T-012–T-015), generation (T-005–T-007), or infrastructure (T-004) is caught by these tests.

**Revision history:** This ticket was revised after TDD review (see `docs/context/tickets/018-integration-tests-full-pipeline.md` revision note). All 4 blocking + 7 moderate issues were resolved. The key architectural constraint driving the revision: `ProjectSpec` only has 2 plugin slots (`backend_id` + `frontend_id`), so the original `all_plugins_spec` fixture was replaced with pairwise combos (`full_spec` = fastapi+react, `django_htmx_spec` = django+htmx).

**New fixtures (added to conftest.py):**

| Fixture | Scope | Purpose | Mock boundaries |
|---------|-------|---------|----------------|
| `qapp` | session | QApplication instance for GUI tests | None (real `QApplication(sys.argv)`) |
| `full_spec` | function | ProjectSpec(fastapi+react) with explicit config | None (real domain model) |
| `django_htmx_spec` | function | ProjectSpec(django+htmx) with explicit config | None (real domain model) |
| `worker` | function | GenerationWorker bound to orchestrator+spec+temp_dir | `mock_executor` (via orchestrator fixture) |

**Test areas and what they exercise:**

| Test file | # Tests | What it exercises | Upstream components tested |
|-----------|---------|-------------------|--------------------------|
| `test_all_plugins.py` | 4 | Multi-plugin pipeline: fastapi+react file generation, django+htmx directory coexistence, custom test-double dependency ordering | **Orchestrator.generate()**, **PluginExecutionEngine** (isinstance dispatch, topo-sort, cancellation), **GenerationTransaction.commit()**, **FastapiPlugin.files()**, **ReactPlugin.files()**, **DjangoPlugin.files()**, **HtmxPlugin.files()** |
| `test_gui_worker.py` | 4 | Worker thread: generation on QThread, cancel→rollback, signal routing via MainWindow, error→finished(success=False) | **GenerationWorker.run()** on QThread, **QtProgressReporter.should_cancel()**, **MainWindow._create_generation_worker()** signal wiring, **MainWindow._on_generation_finished()** |
| `test_overwrite_flow.py` | 3 | Overwrite confirm: dialog shown at index 3, Yes→proceed, No→stay at review | **MainWindow.next_screen()** at index 3, **MainWindow.show_confirm()**, **QMessageBox.question** monkeypatch, **Path.exists()** mocking |
| `test_error_scenarios.py` | 7 | Error/rollback: mid-stage exception, scaffold failure, timeout, missing plugin (CLI+API), empty name, scaffold+files overlap | **Orchestrator.generate()** try/except path, **GenerationTransaction.rollback()**, **ValidationEngine.validate_spec()**, **app.main()** / **pytest.raises(SystemExit)**, **PluginExecutionEngine** with mock side_effect |

**Mock boundary strategy:**

| Component | Real or Mock | Rationale |
|-----------|-------------|-----------|
| `PluginRegistry` | **Real** (`pipeline_registry` fixture) | Loads all 4 production plugins; canary for import errors |
| `ValidationEngine` | **Real** | Wraps real registry; validates actual plugin questions |
| `Orchestrator` | **Real** | Instantiates real 6 stages; exercises full pipeline |
| `GenerationTransaction` | **Real** | Real filesystem staging + commit/rollback via `tmp_path` |
| `ProcessExecutor` | **Mock** (`mock_executor`) | Prevents real subprocess calls (`uv add`, `npm create`) |
| `QApplication` | **Real** (`qapp` fixture) | Required for any QObject-based test (GenerationWorker, MainWindow) |
| `QMessageBox.question` | **Monkeypatched** | Overwrite confirm tests; avoids modal dialog |
| `Path.exists()` / `Path.cwd()` | **Monkeypatched** | Overwrite flow tests; controls output dir state |

**Delicate points specific to T-018:**

| # | Point | Risk |
|---|-------|------|
| 1 | **Dead `GenerationWorker.error` signal** (`workers.py:55`) — `Signal(str)` is defined but **never emitted** in `run()`. T-018 tests `finished(success=False)` instead. Any future developer expecting `error` to fire will be confused. The dead wire must be fixed or documented as intentional. | **High** |
| 2 | **QApp lifecycle on headless CI** — `qapp` fixture creates `QApplication(sys.argv)`. On CI without display, this may crash without `QT_QPA_PLATFORM=offscreen`. GUI tests must be guarded or CI must set the env var. | **High** |
| 3 | **Django config keys mismatch (FIXED)** — Was using `orm`/`auth`/`include_alembic` under `"django"` key (FastAPI's keys). Fixed to use Django's actual keys: `{"database": "sqlite", "include_drf": False}`. | **Medium** — resolved |
| 4 | **Mock executor hides scaffold output** — `test_react_scaffold_files_overlap` tests scaffold+files overlap. Since `mock_executor` makes `generate()` a no-op, only the `files()` side of the overlap is verifiable. Actual create-vite scaffold output is never tested. | **Medium** |
| 5 | **Overwrite flow requires full MainWindow fixture** — must instantiate MainWindow with all 5 real screens, mock `Path.cwd()` + `Path.exists()`, monkeypatch `QMessageBox.question`, and navigate screen 3→4. Complex setup with 3 mock boundaries. | **Medium** |
| 6 | **Multi-plugin tests exercise all 4 real plugin imports** — `pipeline_registry` fixture triggers real `importlib.metadata.entry_points()`. Any broken plugin import crashes all multi-plugin tests. Same canary pattern as T-016/T-017. | **Medium** |
| 7 | **Module-scoped `pipeline_registry` shared with T-017** — both T-017 and T-018 tests import the same module-scoped fixture from conftest. The registry is read-only after discovery, so no state pollution, but ordering is implicit. | **Low** |
| 8 | **`full_spec` config pre-fills ValidationEngine requirements** — if either plugin's required questions change, the fixture config becomes stale but tests still pass silently (config is pre-filled, not validated against changed questions). | **Low** |
| 9 | **Signal routing through MainWindow, not GenerationScreen** — GenerationScreen is **passive** per T-015 design. Tests must verify signal connections through `MainWindow._create_generation_worker()`, not by connecting directly to GenerationScreen. | **Medium** |

**CLI error test dual patterns:**

`test_error_missing_plugin_id` must test two paths with different assertion patterns:

```python
# CLI path: exit code 1 via sys.exit
with pytest.raises(SystemExit) as exc:
    app.main(["--headless", str(spec_json)])
assert exc.value.code == 1

# API path: ValidationError returned
...
- ~18 tests across 4 new test files — all must PASS
- 4 new fixtures in conftest.py coexist with T-016/T-017's existing 13 fixtures
- GUI tests require `QT_QPA_PLATFORM=offscreen` or display
- `test_error_special_chars_in_name` is **DEFERRED** — marked `@pytest.mark.skip(reason="not implemented — requires sanitization")`
- 0 regressions in 42 existing integration tests (T-016 + T-017) and 436+ unit tests

**Implementation status:** ⏳ **Pending** — 4 test files to create, 1 existing file extended (conftest.py), 0 production code changes. Requires T-009 through T-017 to be complete and passing. Ticket verified by TDD review (second round — verdict: APPROVED).
