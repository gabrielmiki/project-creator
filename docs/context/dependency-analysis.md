...
### Integration Test Layer (T-016 ✅, T-017 ⏳ pending)
| File | Action | Purpose | Depends on |
|------|--------|---------|-------------|
| `tests/integration/__init__.py` | ✅ **Created (T-016)** | Package init | None (empty) |
| `tests/integration/conftest.py` | ✅ **Created (T-016)** → **MODIFY (T-017)** | Shared fixtures: `AllMixinsPlugin`, `temp_dir`, `minimal_plugin`, `spec_factory` (reuses `tests/unit/_shared.make_spec`), `user_plugin_dir` (flat .py + subdirectory formats), `registry_with_discovery`, `txn`; T-017 adds 7 new fixtures: `pipeline_registry`, `validation`, `mock_executor`, `progress`, `orchestrator`, `fastapi_spec`, `cli_spec_json` | `forge.domain`, `forge.infrastructure`, `forge.plugins.base`, `forge.generation.registry`, `forge.infrastructure.transaction`, `tests.unit._shared` |
...
| `tests/integration/test_cli_headless.py` | **CREATE (T-017)** | Exercises `app.main()`, `_run_headless()`, `detect_display()`, `Orchestrator.generate()`, `GenerationTransaction.commit()` | `forge.app`, `forge.generation.orchestrator`, `forge.infrastructure.transaction` |
| `tests/integration/test_orchestrator_pipeline.py` | **CREATE (T-017)** | Exercises `Orchestrator.generate()` with real 6 stages, `GenerationTransaction.commit()`/`rollback()`, `MockProgressReporter` | `forge.generation.orchestrator`, `forge.generation.stages.*`, `forge.generation.progress`, `forge.infrastructure.transaction` |
| `tests/integration/test_fastapi_plugin.py` | **CREATE (T-017)** | Exercises `PluginExecutionEngine.run()` with real `FastapiPlugin`, real `GenerationTransaction` on real `temp_dir` | `forge.generation.stages.plugin_execution_engine`, `forge.plugins.fastapi.plugin`, `forge.infrastructure.transaction` |
...
**Implementation status:** ⏳ **Pending** — 3 test files to create, 1 existing file to modify (conftest.py), 0 production code changes. Requires T-005, T-006, T-007, T-008, and T-016 to be complete and passing.
