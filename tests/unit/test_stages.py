...
from forge.plugins.base import PluginBase, CommandRunner, DependencyProvider, FileProvider
...
class _MockFilePlugin(PluginBase, FileProvider):
    name = "mock-file"
    display_name = "Mock File Plugin"
    description = ""

    def __init__(self, files: list[GeneratedFile] | None = None, dirs: list[str] | None = None) -> None:
        super().__init__()
...
class _MockCommandPlugin(PluginBase, CommandRunner):
    name = "mock-command"
...
class _MockDepPlugin(PluginBase, DependencyProvider):
    name = "mock-dep"
...
def _make_spec(backend_id: str = "", frontend_id: str | None = None) -> ProjectSpec:
    return ProjectSpec(
        project_name="test-proj",
        template=TemplateDefinition(
            id="test", display_name="Test Template", description="",
            backend_id=backend_id, frontend_id=frontend_id,
        ),
...
def _make_empty_spec() -> ProjectSpec:
    return ProjectSpec(
        project_name="empty",
...
def _build_registry(plugins: list[PluginBase]) -> MagicMock:
...
class TestDirectoryInitializer:
    ...
    def test_empty_directory_passes(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
    ...
    def test_non_existent_directory_passes(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.directory_initializer import DirectoryInitializer

        DirectoryInitializer().run(spec, output_dir, txn, progress)

    def test_directory_with_dotfile_passes(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.directory_initializer import DirectoryInitializer
    ...
    def test_file_in_dir_raises(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.directory_initializer import DirectoryInitializer
    ...
    def test_subdirectory_raises(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.directory_initializer import DirectoryInitializer
        from forge.generation.errors import DirectoryNotEmptyError
    ...
    def test_nested_content_raises(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.directory_initializer import DirectoryInitializer
        from forge.generation.errors import DirectoryNotEmptyError
...
class TestSharedStructureScaffolder:
    ...
    def test_shared_files_created(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
    ...
    def test_docs_stub_created(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.shared_structure_scaffolder import SharedStructureScaffolder
    ...
    def test_empty_spec_still_generates(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, empty_spec: ProjectSpec) -> None:
        from forge.generation.stages.shared_structure_scaffolder import SharedStructureScaffolder
...
class TestPluginExecutionEngine:
    ...
    def test_file_provider_forwards_files(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="fp")
        files = [
            GeneratedFile(Path("main.py"), "print(1)"),
            GeneratedFile(Path("utils.py"), "def foo(): pass"),
        ]
        dirs = ["static", "templates"]
        plugin = _MockFilePlugin(files=files, dirs=dirs)
        plugin.name = "fp"
        registry = _build_registry([plugin])
    ...
    def test_file_provider_no_files(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="fp")
        plugin = _MockFilePlugin(files=[], dirs=[])
        plugin.name = "fp"
        registry = _build_registry([plugin])
    ...
    def test_generated_file_executable_flag(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="fp")
        files = [GeneratedFile(Path("script.sh"), "#!/bin/bash\necho hi", executable=True)]
        plugin = _MockFilePlugin(files=files, dirs=[])
        plugin.name = "fp"
        registry = _build_registry([plugin])
    ...
    def test_command_runner_generate_called(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="cp")
        plugin = _MockCommandPlugin()
        plugin.name = "cp"
        registry = _build_registry([plugin])
    ...
    def test_command_runner_multiple_both_called(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="cp1", frontend_id="cp2")
        p1 = _MockCommandPlugin()
        p1.name = "cp1"
        p2 = _MockCommandPlugin()
        p2.name = "cp2"
        registry = _build_registry([p1, p2])
    ...
    def test_command_runner_adds_checkpoint(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine
    ...
    def test_missing_dep_raises(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine
        from forge.generation.errors import MissingDependencyError

        spec = _make_spec(backend_id="needy")
        plugin = _MockFilePlugin(files=[], dirs=[])
        plugin.name = "needy"
    ...
    def test_present_dep_passes(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="a", frontend_id="b")
        a = _MockFilePlugin(files=[], dirs=[])
        a.name = "a"
        a.requires = ["b"]
        b = _MockFilePlugin(files=[], dirs=[])
        b.name = "b"
        registry = _build_registry([a, b])

        PluginExecutionEngine(registry).run(spec, output_dir, txn, progress)

    def test_empty_requires_passes(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="empty")
        plugin = _MockFilePlugin(files=[], dirs=[])
        plugin.name = "empty"
        plugin.requires = []
        registry = _build_registry([plugin])
    ...
    def test_circular_dependency_raises(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine
    ...
    def test_self_referencing_dep_raises(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine
        from forge.generation.registry import CycleDependencyError

        spec = _make_spec(backend_id="self")
        registry: MagicMock = MagicMock()
    ...
    def test_no_cycle_passes(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine
    ...
    def test_dep_appended_to_requirements(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="dp")
        plugin = _MockDepPlugin(deps=["fastapi", "sqlalchemy"])
        plugin.name = "dp"
        registry = _build_registry([plugin])
    ...
    def test_multiple_dep_providers_accumulate(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="d1", frontend_id="d2")
        d1 = _MockDepPlugin(deps=["pydantic"])
        d1.name = "d1"
        d2 = _MockDepPlugin(deps=["uvicorn"])
        d2.name = "d2"
        registry = _build_registry([d1, d2])
    ...
    def test_empty_deps_appends_nothing(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="dp")
        plugin = _MockDepPlugin(deps=[])
        plugin.name = "dp"
        registry = _build_registry([plugin])
    ...
    def test_cancellation_stops_after_first(self, output_dir: Path, txn: _MockTransaction, progress: _CancellableReporter) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="p1", frontend_id="p2")
        p1 = _MockCommandPlugin()
        p1.name = "p1"
        p2 = _MockCommandPlugin()
        p2.name = "p2"
    ...
    def test_cancellation_before_any_plugin(self, output_dir: Path, txn: _MockTransaction, progress: _CancellableReporter) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine
    ...
    def test_no_cancellation_runs_all(self, output_dir: Path, txn: _MockTransaction, progress: _CancellableReporter) -> None:
        from forge.generation.stages.plugin_execution_engine import PluginExecutionEngine

        spec = _make_spec(backend_id="p1", frontend_id="p2")
        p1 = _MockDepPlugin(deps=["lib-a"])
        p1.name = "p1"
        p2 = _MockDepPlugin(deps=["lib-b"])
        p2.name = "p2"
        registry = _build_registry([p1, p2])
...
class TestJustfileGenerator:
    ...
    def test_justfile_contains_default_commands(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
    ...
    def test_justfile_called_via_stage_file(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.justfile_generator import JustfileGenerator
    ...
    def test_justfile_no_domains_still_has_commands(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, empty_spec: ProjectSpec) -> None:
        from forge.generation.stages.justfile_generator import JustfileGenerator
...
class TestProjectDocumentationWriter:
    ...
    def test_agents_md_contains_project_name(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
    ...
    def test_claude_md_exists(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.project_documentation_writer import ProjectDocumentationWriter
    ...
    def test_empty_spec_still_generates_both_files(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, empty_spec: ProjectSpec) -> None:
        from forge.generation.stages.project_documentation_writer import ProjectDocumentationWriter
...
class TestAgentSkillScaffolder:
    ...
    def test_all_three_directories_created(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
    ...
    def test_directories_created_via_stage_directory(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, spec: ProjectSpec) -> None:
        from forge.generation.stages.agent_skill_scaffolder import AgentSkillScaffolder
    ...
    def test_no_plugins_creates_default_stubs(self, output_dir: Path, txn: _MockTransaction, progress: MagicMock, empty_spec: ProjectSpec) -> None:
        from forge.generation.stages.agent_skill_scaffolder import AgentSkillScaffolder
